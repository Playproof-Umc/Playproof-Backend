...
} from '../dtos/community-post.res.dto';
import {
  CommunityPostCreateReqDto,
  CommunityPostUpdateReqDto,
} from '../dtos/community-post.req.dto';
...
export class CommunityPostController extends Controller {
  ...
  public async getMyPostList(
    ...
    const result = await this.service.getMyPostList(req.user.id, cursor ?? null, size);
    this.setStatus(result.statusCode);
    return result;
  }
