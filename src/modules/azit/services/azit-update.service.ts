// src/modules/azit/services/azit-update.service.ts
...
import { Result, ok, conflict, internalServerError, notFound, forbidden } from "../../../common/types/result.type";
import { uploadFileToS3, deleteFileFromS3 } from "../../../common/utils/file-util";
import { AzitUserRole } from "@prisma/client";

@injectable()
export class AzitUpdateService {
  constructor(
  ...
  async updateAzit(
    ...
    const userRole = await this.azitUserRepository.findAzitUserRoleByUserIdAndAzitId(userId, azitId);
    ...
    if (dto.azit_name !== null && dto.azit_name !== undefined) {
      ...
      if (existingAzitNames.filter(name => name !== azit.azitName).includes(dto.azit_name)) {
        return conflict({
